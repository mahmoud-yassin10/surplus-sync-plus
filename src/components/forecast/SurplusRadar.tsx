      ...
      <div className="grid lg:grid-cols-[1fr_260px] gap-4 items-start">
        <div className="relative aspect-square w-full max-w-[440px] mx-auto">
          <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-full">
            {[60, 110, 160].map((r, i) => (
              <g key={r}>
                <circle cx={cx} cy={cy} r={r} fill="none" stroke="var(--color-line)" strokeDasharray="2 4" />
                <text x={cx + r + 3} y={cy + 3} fontSize="9" fill="var(--color-text-faint)">+{(i + 1) * 5}d</text>
              </g>
            ))}

            <circle cx={cx} cy={cy} r="26" fill="var(--color-ink)" />
            <text x={cx} y={cy - 2} textAnchor="middle" fontSize="9" fill="white" opacity="0.7">LINCOLN</text>
            <text x={cx} y={cy + 9} textAnchor="middle" fontSize="9" fill="white" opacity="0.7">HEIGHTS</text>
            ...
            {days.map((d, i) => {
              ...
              return (
                <g key={d.date}>
                  <line x1={cx} y1={cy} x2={x} y2={y} stroke="var(--color-line)" strokeWidth="0.6" opacity="0.4" />
                  {isFocus && (
                    <circle cx={x} cy={y} r={sz + 6} fill={color} opacity="0.18">
                      <animate attributeName="r" values={`${sz + 6};${sz + 14};${sz + 6}`} dur="2.4s" repeatCount="indefinite" />
                      <animate attributeName="opacity" values="0.22;0.05;0.22" dur="2.4s" repeatCount="indefinite" />
                    </circle>
                  )}
                  <circle cx={x} cy={y} r={sz} fill={color} />
                  <text x={x} y={y - sz - 4} textAnchor="middle" fontSize="8.5" fill="var(--color-text-soft)">{d.label} {d.date.slice(8)}</text>
                </g>
              );
            })}

            {state.partners.filter((p) => p.status !== "closed").map((p, i, arr) => {
              ...
              return (
                <g key={p.id}>
                  <path d={`M ${cx} ${cy} L ${x} ${y}`} stroke="var(--color-success)" strokeWidth="0.8" strokeDasharray="3 4" opacity="0.35" />
                  <circle cx={x} cy={y} r="6" fill="var(--color-success-soft)" stroke="var(--color-success)" />
                  <text x={x} y={y + 14} textAnchor="middle" fontSize="8" fill="var(--color-text-soft)">{p.capacity}</text>
                </g>
              );
            })}
          </svg>
        ...
        <aside className="space-y-3">
          ...
          <div className="rounded-md border border-[var(--color-line)] p-3">
            ...
            <div className="space-y-1.5">
              {state.partners.filter((p) => p.status === "available").map((p) => (
                <div key={p.id} className="flex items-center justify-between text-[11.5px]">
                  ...
                  <span className="tnum text-[var(--color-text-soft)] whitespace-nowrap">{p.capacity} · {p.distanceMi}mi</span>
                </div>
              ))}
            </div>
          </div>
...
function Row({ label, value, sub, tone }: { label: string; value: string; sub?: string; tone?: "ai" | "critical" }) {
  const color = tone === "ai" ? "text-[var(--color-ai)]" : tone === "critical" ? "text-[var(--color-critical)]" : "text-[var(--color-text)]";
  return (
