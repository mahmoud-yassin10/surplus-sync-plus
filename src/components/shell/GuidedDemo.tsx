...
export function GuidedDemo() {
  ...
  return (
    <>
      <button onClick={() => dispatch({ type: "GUIDED_STEP", step: 1 })} className="text-[11px] flex items-center gap-1 px-2 py-1 rounded-md bg-[var(--color-ink)] text-white hover:opacity-90">
      ...
      {active && step && (
        <div className="fixed bottom-5 right-5 z-50 w-[340px] rounded-lg border border-[var(--color-line)] bg-[var(--color-surface)] shadow-xl overflow-hidden">
          <div className="px-4 py-2.5 border-b border-[var(--color-line)] flex items-center bg-[var(--color-ink)] text-white">
            <div className="text-[10.5px] uppercase tracking-[0.16em] opacity-70">Guided demo · step {state.guidedStep} of {STEPS.length}</div>
            <button onClick={() => dispatch({ type: "GUIDED_STEP", step: 0 })} className="ml-auto opacity-70 hover:opacity-100">
              <X size={14} />
            </button>
          </div>
          <div className="px-4 py-3.5">
            <div className="text-[13.5px] font-semibold mb-1">{step.title}</div>
            <p className="text-[12px] text-[var(--color-text-soft)] leading-relaxed">{step.description}</p>
          </div>
          <div className="px-4 py-3 border-t border-[var(--color-line)] flex gap-2">
            <button onClick={() => dispatch({ type: "GUIDED_STEP", step: Math.max(1, state.guidedStep - 1) })} className="text-[11.5px] px-2.5 py-1.5 rounded-md border border-[var(--color-line)] hover:bg-[var(--color-surface-2)]">Back</button>
            <button
              onClick={() => {
                step.action?.(dispatch);
                dispatch({ type: "GUIDED_STEP", step: state.guidedStep < STEPS.length ? state.guidedStep + 1 : 0 });
              }}
