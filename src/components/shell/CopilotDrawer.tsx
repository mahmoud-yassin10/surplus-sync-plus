    ...
    <aside className="fixed right-0 top-0 bottom-0 w-full sm:w-[420px] bg-[var(--color-surface)] border-l border-[var(--color-line)] z-40 flex flex-col shadow-2xl">
            ...
            <div className="space-y-1.5">
              {PROMPTS.map((p) => (
                <button
                  key={p}
                  onClick={() => send(p)}
                  className="w-full text-left text-[12px] px-3 py-2 rounded-md border border-[var(--color-line)] hover:border-[var(--color-ai)]/40 hover:bg-[var(--color-ai-soft)]/40 transition flex items-center gap-2"
                >
        ...
        {thread.map((t, i) => (
          <div key={i} className="space-y-2">
            ...
            <div className="rounded-md border border-[var(--color-line)] bg-[var(--color-surface)] p-3">
              ...
              <div className="text-[12.5px] font-medium text-[var(--color-text)] mb-1.5">
                {t.r.title}
              </div>
              <p className="text-[12px] text-[var(--color-text-soft)] leading-relaxed">
                {t.r.body}
              ...
              {t.r.evidence && (
                <ul className="mt-2.5 space-y-1">
                  {t.r.evidence.map((e) => (
                    ...
                    >
                      <span className="mt-1 h-1 w-1 rounded-full bg-[var(--color-text-faint)]" />{" "}
                      {e}
                    </li>
                  ))}
                </ul>
              ...
              {t.r.proposal && (
                <div className="mt-3 rounded-md border border-[var(--color-ai)]/30 bg-[var(--color-ai-soft)]/40 p-2.5 text-[11.5px]">
                  ...
                  <div className="text-[var(--color-text-soft)] mb-2">
                    {t.r.proposal.consequences}
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button className="text-[11px] px-2.5 py-1 rounded border border-[var(--color-line)]">
                      Edit
                    </button>
                    <button className="text-[11px] px-2.5 py-1 rounded border border-[var(--color-line)] text-[var(--color-critical)]">
                      Reject
                    </button>
                  </div>
                </div>
      ...
      >
        <input
          ...
          className="flex-1 text-[12.5px] px-3 py-2 rounded-md border border-[var(--color-line)] bg-[var(--color-surface)] focus:outline-none focus:border-[var(--color-ai)]"
        />
        <button className="text-[12px] px-3 py-2 rounded-md bg-[var(--color-ai)] text-white">
          Send
        </button>
      </form>
    </aside>
