...
export function ApprovalGate({
  ...
  return (
    <div className={`rounded-md border p-4 ${approved ? "border-[var(--color-success)]/30 bg-[var(--color-success-soft)]/30" : "border-[var(--color-ai)]/30 bg-[var(--color-ai-soft)]/40"}`}>
      <div className="flex items-start gap-3">
        <div className={`mt-0.5 h-7 w-7 rounded-full flex items-center justify-center shrink-0 ${approved ? "bg-[var(--color-success)] text-white" : "bg-[var(--color-ai)] text-white"}`}>
          {approved ? <CheckCircle2 size={14} /> : <ShieldAlert size={14} />}
        </div>
        <div className="min-w-0 flex-1">
          <div className="text-[10.5px] uppercase tracking-[0.14em] text-[var(--color-text-faint)]">Human approval gate</div>
          <div className="text-[14px] font-semibold mt-0.5">{title}</div>
          <div className="text-[11.5px] text-[var(--color-text-soft)] mt-0.5">Required role: <span className="text-[var(--color-text)]">{who}</span></div>
          ...
          <div className="mt-3 flex items-center gap-2 flex-wrap">
            {!approved && (
              <>
                ...
                <button className="text-[11.5px] px-3 py-1.5 rounded-md border border-[var(--color-line)]">Modify</button>
              </>
            )}
            {approved && reversible && onUndo && (
              <button onClick={onUndo} className="text-[11.5px] px-3 py-1.5 rounded-md border border-[var(--color-line)] flex items-center gap-1">
                <Undo2 size={12} /> Undo
              </button>
            )}
            <span className="ml-auto text-[10.5px] text-[var(--color-text-faint)]">{reversible ? "Reversible action" : "Permanent action"}</span>
          </div>
