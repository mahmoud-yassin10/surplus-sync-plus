  ...
  return (
    ...
    >
      <div className="grid lg:grid-cols-[1.5fr_1fr] gap-5">
        <Section title="Operational map" hint="Dashed arcs = provisional · solid = confirmed · teal = completed">
          <div className="p-4"><NetworkMap onSelect={setSelected} selectedId={selected} /></div>
        </Section>
          ...
          {partner && (
            <div className="p-4 space-y-3 text-[12.5px]">
              <div className="flex items-center gap-2 text-[var(--color-text-soft)]">
                <span className={`h-1.5 w-1.5 rounded-full ${partner.status === "available" ? "bg-[var(--color-success)]" : partner.status === "limited" ? "bg-[var(--color-warning)]" : "bg-[var(--color-text-faint)]"}`} />
              ...
              <div className="grid grid-cols-2 gap-3 stagger-fast">
                ...
                <Cell label="Own vehicle" value={partner.vehicle ? "Yes" : "No"} icon={<Truck size={11} />} />
              </div>
              <div className="rounded-md bg-[var(--color-surface-2)] p-3 text-[11.5px]">
                <div className="text-[10px] uppercase tracking-wider text-[var(--color-text-faint)] mb-1">Accepts</div>
            ...
            <table className="w-full text-[12.5px] min-w-[640px]">
              ...
              <tbody>
                {state.partners.map((p) => (
                  <tr key={p.id} className={`border-b border-[var(--color-line)] last:border-0 cursor-pointer hover:bg-[var(--color-surface-2)]/40 ${selected === p.id ? "bg-[var(--color-ai-soft)]/30" : ""}`} onClick={() => setSelected(p.id)}>
                    ...
                    <td className="px-4 py-2.5 tnum text-[var(--color-text-soft)]">{p.windowStart}–{p.windowEnd}</td>
                    <td className="px-4 py-2.5 text-[var(--color-text-soft)]">{p.accepts.join(", ")}</td>
                    <td className="px-4 py-2.5 text-[var(--color-text-soft)]">{[p.refrigerated && "refrigerated", p.vehicle && "vehicle"].filter(Boolean).join(", ") || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
