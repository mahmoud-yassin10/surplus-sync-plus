  ...
  return (
    <Page kicker="School calendar" title="March 2026">
      <div className="grid lg:grid-cols-[1.6fr_1fr] gap-5">
        <Section title="Month view" hint="Days with risk overlays are calendar inputs to the forecast">
          <div className="p-4">
            ...
            <div className="grid grid-cols-7 gap-1">
              {cells.map((c, i) => {
                ...
                return (
                  <div key={c.date} className={`aspect-square rounded border p-1.5 text-[10.5px] flex flex-col ${
                    isFocus ? "border-[var(--color-critical)] bg-[var(--color-critical-soft)]/30"
                    : isWeekend ? "border-[var(--color-line)] bg-[var(--color-surface-2)]/40 text-[var(--color-text-faint)]"
                    : events.length > 0 ? "border-[var(--color-warning)]/40 bg-[var(--color-warning-soft)]/30"
                    ...
                    <div className="mt-auto flex flex-wrap gap-0.5">
                      {events.slice(0, 3).map((e) => {
                        const I = ICON[e.kind];
                        return I ? <I key={e.id} size={9} className="text-[var(--color-text-soft)]" /> : null;
                      })}
                    </div>
                  </div>
                );
                ...
                <li key={e.id} className="px-4 py-3 flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[10.5px] text-[var(--color-text-faint)] mt-0.5">
                      {e.grades ? `Grades ${e.grades.join(", ")} · ` : ""}{e.confidence} confidence
                    </div>
                    {e.notes && <div className="text-[11px] text-[var(--color-text-soft)] mt-1">{e.notes}</div>}
                  </div>
                  <span className={`tnum text-[11.5px] font-medium ${e.attendanceDelta < 0 ? "text-[var(--color-critical)]" : "text-[var(--color-success)]"}`}>
                    {e.attendanceDelta > 0 ? "+" : ""}{e.attendanceDelta}
                  </span>
                </li>
              );
            })}
