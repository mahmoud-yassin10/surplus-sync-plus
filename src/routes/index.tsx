...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Operational meal forecasting and food-waste prevention for school cafeterias." },
    ],
  }),
  ...
  return (
    <Page kicker="School Command Center" title="Thursday Mar 12 is a high-risk surplus event">
      ...
      <div className="grid lg:grid-cols-[1.5fr_1fr] gap-5">
            ...
            <ul className="divide-y divide-[var(--color-line)]">
              {f.influences.map((i) => (
                <li key={i.factor} className="px-4 py-3 grid grid-cols-[20px_1fr_auto] gap-3 items-center">
                  <span className={`h-2 w-2 rounded-full ${i.direction === "down" ? "bg-[var(--color-critical)]" : "bg-[var(--color-success)]"}`} />
                  <div>
                    <div className="text-[12.5px] text-[var(--color-text)]">{i.factor}</div>
                    <div className="text-[11px] text-[var(--color-text-faint)]">{i.note}</div>
                  </div>
                  <span className="tnum text-[11.5px] text-[var(--color-text-soft)]">{i.direction === "down" ? "−" : "+"}{i.magnitude}</span>
                </li>
              ))}
            </ul>
          </Section>
          ...
          <Section title="Current plan vs AI recommendation" hint="Uncertainty shown as the 80% prediction interval">
            <div className="p-5">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                <Link to="/forecast" className="text-[12px] px-3 py-2 rounded-md border border-[var(--color-line)]">Full daily forecast →</Link>
              </div>
            </div>
          </Section>
                ...
                <li key={m.item} className="px-4 py-2.5 flex items-center">
                  ...
                  <div className="ml-auto flex items-center gap-3">
                    <div className="w-32 h-1 rounded-full bg-[var(--color-line)] hidden sm:block">
                      <div className="h-1 rounded-full bg-[var(--color-ai)]" style={{ width: `${(m.recommended / 600) * 100}%` }} />
                    </div>
                    <span className="tnum text-[12.5px] font-medium w-12 text-right">{m.recommended}</span>
                  </div>
                </li>
              ))}
            </ul>
        ...
        {/* Right column */}
        <div className="space-y-5">
          <Section title="Partner readiness" hint="Capacity ready before surplus is confirmed" right={<Link to="/recovery" className="text-[11.5px] text-[var(--color-ai)] inline-flex items-center gap-0.5">Open <ArrowUpRight size={11} /></Link>}>
            <ul className="divide-y divide-[var(--color-line)]">
              {state.partners.slice(0, 4).map((p) => {
                ...
                return (
                  <li key={p.id} className="px-4 py-2.5 flex items-center gap-2">
                    <span className={`h-1.5 w-1.5 rounded-full ${p.status === "available" ? "bg-[var(--color-success)]" : p.status === "limited" ? "bg-[var(--color-warning)]" : "bg-[var(--color-text-faint)]"}`} />
                    <div className="min-w-0">
                      <div className="text-[12.5px] truncate text-[var(--color-text)]">{p.name}</div>
                      <div className="text-[10.5px] text-[var(--color-text-faint)] tnum">{p.capacity} meals · {p.distanceMi}mi · {p.windowStart}–{p.windowEnd}</div>
                    </div>
                    <span className="ml-auto text-[10.5px]">
                      {match?.state === "reserved" && <span className="px-1.5 py-0.5 rounded bg-[var(--color-ai-soft)] text-[var(--color-ai)]">Reserved {match.reservedMeals}</span>}
                      {match?.state === "confirmed" && <span className="px-1.5 py-0.5 rounded bg-[var(--color-success-soft)] text-[var(--color-success)]">Confirmed</span>}
                    </span>
                  </li>
                );
              })}
          ...
          <Section title="Copilot insight" right={<Sparkles size={13} className="text-[var(--color-ai)]" />}>
            <div className="p-4 text-[12.5px] text-[var(--color-text-soft)] leading-relaxed">
              ...
              <div className="mt-3 flex gap-2">
                <EvidenceTrigger />
                <Link to="/forecast" className="text-[12px] px-3 py-1.5 rounded-md border border-[var(--color-line)]">Ask Copilot</Link>
              </div>
            </div>
          </Section>

          <Section title="Impact ledger" hint="Numbers stay separated — prototype data" right={<Link to="/impact" className="text-[11.5px] text-[var(--color-ai)] inline-flex items-center gap-0.5">Open <ArrowUpRight size={11} /></Link>}>
            <div className="p-4 grid grid-cols-2 gap-3">
              <Ledger label="Prevented" value={state.impact.preventedMeals} tone="ai" sub="meals not prepared" />
              <Ledger label="Recovered" value={state.impact.recoveredMeals} tone="success" sub="safe untouched" />
              <Ledger label="Nonrecoverable" value={state.impact.wastedMeals} tone="critical" sub="cannot redistribute" />
              <Ledger label="Cost saved" value={`$${Math.round(state.impact.costSaved)}`} sub="procurement avoided" />
            </div>
          </Section>
              ...
              {state.audit.slice(0, 4).map((a) => (
                <li key={a.id} className="px-4 py-2.5 flex items-start gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[10.5px] text-[var(--color-text-faint)]">{a.actor} · {a.actorType}</div>
                  </div>
                </li>
              ))}
            </ul>
...
function Ledger({ label, value, sub, tone }: { label: string; value: any; sub: string; tone?: "ai" | "success" | "critical" }) {
  const c = tone === "ai" ? "text-[var(--color-ai)]" : tone === "success" ? "text-[var(--color-success)]" : tone === "critical" ? "text-[var(--color-critical)]" : "text-[var(--color-text)]";
  return (
  ...
  return (
    <div>
      <div className="relative h-14">
        ...
        <div className="absolute inset-y-6 h-2 rounded-full bg-[var(--color-ai-soft)]" style={{ left: pct(low), width: `calc(${pct(high)} - ${pct(low)})` }} />

        <Marker pos={pct(expected)} color="var(--color-ai)" top label={`${expected} expected`} />
        <Marker pos={pct(current)} color="var(--color-critical)" bottom label={`${current} current plan`} />
        <Marker pos={pct(recommended)} color="var(--color-success)" bottom label={`${recommended} recommended`} />
      </div>
...
function Marker({ pos, color, label, top, bottom }: { pos: string; color: string; label: string; top?: boolean; bottom?: boolean }) {
  return (
    <div className="absolute" style={{ left: pos, transform: "translateX(-50%)", top: top ? 0 : undefined, bottom: bottom ? 0 : undefined }}>
      <div className="w-px h-7 mx-auto" style={{ background: color }} />
      <div className="text-[10px] whitespace-nowrap tnum text-center" style={{ color }}>{label}</div>
    </div>
  );
}
